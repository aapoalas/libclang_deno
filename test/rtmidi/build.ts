...
const includeDirectory = join(dirname(fromFileUrl(import.meta.url)));

const includePaths = [
  "-I/lib64/clang/14.0.6/include/",
  `-I${includeDirectory}`,
];
...
HEADER_FILES.forEach((fileName) => {
  ...
  const fullPathName = join(includeDirectory, fileName);
...
{
  const INDEX_FUNCTIONS = FUNCTIONS_MAP.get("rtmidi_c.h")!;
}
...
Deno.mkdirSync("rtmidi_output", { recursive: true });
Deno.writeTextFileSync("rtmidi_output/typeDefinitions.ts", results.join("\n"));
formatSync("./rtmidi_output/typeDefinitions.ts");
...
`);
  ...
  if (functionResults.length) {
    Deno.writeTextFileSync(
      `rtmidi_output/${fileName}.ts`,
      functionResults.join("\n"),
    );
    formatSync(`./rtmidi_output/${fileName}.ts`);
  }
}
